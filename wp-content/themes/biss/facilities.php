<?php include("header.php"); ?>

	<section class="global_banner">
		<img src="assets/img/banner/facilities.jpg" />
		<div class="banner_desc">
			<div class="container">
				<div class="row">
				<div class="col-xs-12">
					<h3>Facilities</h3>
					<p>Dengan kurikulum khusus yang mampu memberikan keseimbangan antara aspek seni dan aspek mental, Sekolah Bina Seni Suara memiliki tujuan utama menumbuhkan pengetahuan teori dan praktek bermusik melalui perhatian khusus dan istimewa.</p>
				</div>
				</div>
			</div>
		</div>
	</section>
	
	<section id="global_title" class="container">
		<div class="row">
			<div class="col-xs-12">
				<h2>Facilities</h2>
			</div>
		</div>
	</section>
	
	<section id="tab_nav" class="container">
		<div class="row">
			<a href="#tab_cont_1" class="col-xs-4">
				<img src="assets/img/facilities/tab1.png" />
				<p>Sewa Studio</p>
			</a>
			<a href="#tab_cont_2" class="col-xs-4">
				<img src="assets/img/facilities/tab2.png" />
				<p>Function Hall</p>
			</a>
			<a href="#tab_cont_3" class="col-xs-4">
				<img src="assets/img/facilities/tab3.png" />
				<p>Restaurant</p>
			</a>			
		</div>
	</section>
	
	<section id="tab_cont_1" class="tab post">
		<div class="width100">
			<div class="container">
			<div class="row">
				<div class="col-md-12">
					<p>Kurikulum vokal disusun sebagai pedoman pendidikan vokal yang sistematis, nyata dan terarah. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla at justo convallis eros dictum pharetra. Sed felis odio, blandit non purus non, malesuada dictum tortor.</p>
				</div>
			</div>
			</div>
		</div>
		<div class="container">
			<div class="row">
				<div class="col-md-12 padnol">
					<img class="long_thumb" src="assets/img/facilities/long_thumb.png" />		
				</div>
				<div class="col-md-12">
					<h3>Recording</h3>
					<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec felis mi, pretium vel aliquet quis, tristique non mi. Donec ante diam, ultrices a augue vel, mollis adipiscing arcu. Curabitur bibendum lobortis volutpat. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus ut erat euismod, condimentum urna eu, placerat arcu. Phasellus eu condimentum est, id scelerisque enim. Proin eget vestibulum ligula, auctor pulvinar eros. </p>
					<h3>Latihan</h3>
					<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec felis mi, pretium vel aliquet quis, tristique non mi. Donec ante diam, ultrices a augue vel, mollis adipiscing arcu. Curabitur bibendum lobortis volutpat. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus ut erat euismod, condimentum urna eu, placerat arcu. Phasellus eu condimentum est, id scelerisque enim. Proin eget vestibulum ligula, auctor pulvinar eros. </p>			
				</div>				
			</div>
		</div>
	</section>
	
	<section id="tab_cont_2" class="tab post">
		<div class="width100">
			<div class="container">
			<div class="row">
				<div class="col-md-12">
					<p>Kurikulum vokal disusun sebagai pedoman pendidikan vokal yang sistematis, nyata dan terarah. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla at justo convallis eros dictum pharetra. Sed felis odio, blandit non purus non, malesuada dictum tortor.</p>
				</div>
			</div>
			</div>
		</div>
		<div class="container">
			<div class="row">
				<div class="col-md-12 padnol">
					<img class="long_thumb" src="assets/img/facilities/long_thumb.png" />		
				</div>
				<div class="col-md-12">
					<h3>Recording</h3>
					<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec felis mi, pretium vel aliquet quis, tristique non mi. Donec ante diam, ultrices a augue vel, mollis adipiscing arcu. Curabitur bibendum lobortis volutpat. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus ut erat euismod, condimentum urna eu, placerat arcu. Phasellus eu condimentum est, id scelerisque enim. Proin eget vestibulum ligula, auctor pulvinar eros. </p>
					<h3>Latihan</h3>
					<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec felis mi, pretium vel aliquet quis, tristique non mi. Donec ante diam, ultrices a augue vel, mollis adipiscing arcu. Curabitur bibendum lobortis volutpat. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus ut erat euismod, condimentum urna eu, placerat arcu. Phasellus eu condimentum est, id scelerisque enim. Proin eget vestibulum ligula, auctor pulvinar eros. </p>			
				</div>				
			</div>
		</div>
	</section>
	
	<section id="tab_cont_3" class="tab post">
		<div class="width100">
			<div class="container">
			<div class="row">
				<div class="col-md-12">
					<p>Kurikulum vokal disusun sebagai pedoman pendidikan vokal yang sistematis, nyata dan terarah. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla at justo convallis eros dictum pharetra. Sed felis odio, blandit non purus non, malesuada dictum tortor.</p>
				</div>
			</div>
			</div>
		</div>
		<div class="container">
			<div class="row">
				<div class="col-md-12 padnol">
					<img class="long_thumb" src="assets/img/facilities/long_thumb.png" />		
				</div>
				<div class="col-md-12">
					<h3>Recording</h3>
					<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec felis mi, pretium vel aliquet quis, tristique non mi. Donec ante diam, ultrices a augue vel, mollis adipiscing arcu. Curabitur bibendum lobortis volutpat. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus ut erat euismod, condimentum urna eu, placerat arcu. Phasellus eu condimentum est, id scelerisque enim. Proin eget vestibulum ligula, auctor pulvinar eros. </p>
					<h3>Latihan</h3>
					<p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec felis mi, pretium vel aliquet quis, tristique non mi. Donec ante diam, ultrices a augue vel, mollis adipiscing arcu. Curabitur bibendum lobortis volutpat. Interdum et malesuada fames ac ante ipsum primis in faucibus. Vivamus ut erat euismod, condimentum urna eu, placerat arcu. Phasellus eu condimentum est, id scelerisque enim. Proin eget vestibulum ligula, auctor pulvinar eros. </p>			
				</div>				
			</div>
		</div>
	</section>

<?php include("footer.php"); ?>	